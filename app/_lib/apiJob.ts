...
export const getJob = (id: string) => async (): Promise<JobsResponse> => {
  ...
  const apiUrl = new URL(
    `https://recruit.zoho.com/recruit/v2/Job_Openings/${id}`,
  );
  ...
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/api/jobs/${id}${apiUrl.search}`,
    {
      method: "GET",
      headers: {
